...
public partial class ShoppingCartGo : PageBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            var oc = this.cookiesHelper.CurrOrderCode;
            var cartTempList = db.tb_cart_temp
                                 .Where(p => p.cart_temp_code.HasValue  && 
                                             p.cart_temp_code.Value.Equals(oc))
                                 .ToList();
            ...
            var hostName = string.Format("{0}{1}/"
                , Request.Url.Host.ToLower().ToString().IndexOf("lucomputers.com") > -1 ? "https://www.lucomputers.com" : string.Concat("http://", Request.Url.Host)
                , Request.Url.Port == 80 ? "" : string.Concat(":", Request.Url.Port));

            if (ReqPayment == setting.PaymentPaypal)
            {
                Response.Redirect(hostName + "ShoppingCartGoView.aspx?auto=1&istopaypal=1", true);
            }
            ...
            {
                Response.Redirect(hostName + "checkoutpickup.aspx?shippingid=" + ReqShipCompany + "&stateid=" + ReqStateID + "&payment=" + ReqPayment, true);
            }
