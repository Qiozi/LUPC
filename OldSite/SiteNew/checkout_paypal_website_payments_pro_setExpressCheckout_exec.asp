...
'----------------------------------------------------------------------------
        ...
        orderCode           = Request.Form("orderCode")
' 		response.write currencyCodeType
'		response.End()
'---------------------------------------------------------------------------
...
'---------------------------------------------------------------------------
		returnURL	= url & "checkout_paypal_website_payments_pro_setExpressCheckout_exec.asp?currencyCodeType=" &  currencyCodeType & _
					"&paymentAmount=" & paymentAmount & _ 
					"&paymentType=" &paymentType &_
                    "&orderCode="&orderCode &_
                    "&EMAIL="&EMAIL
		cancelURL	= url & "checkout_paypal_website_payments_pro_setExpressCheckout.asp?pay=0&paymentType="&paymentType&"&orderCode="&orderCode
...
'---------------------------------------------------------------------------
		...
		SESSION("PayerID")			= Request.Querystring("PayerID")
   		Session("ebay_email")		= Request.querystring("EMAIL")
        Session("orderCode")        = Request.QueryString("orderCode")
 '---------------------------------------------------------------------------
...
'---------------------------------------------------------------------------
	...
	If UCase(ack)="SUCCESS" Then
		Response.Redirect "checkout_paypal_website_payments_pro_getExpressCheckoutDetail.asp"
	Else  
...
End If
	...
	SESSION("ErrorMessage")	= ErrorFormatter(Err.Description,Err.Number,Err.Source,"checkout_paypal_website_payments_pro_setExpressCheckout_exec.asp")
	Response.Redirect "APIError.asp"
	Else
