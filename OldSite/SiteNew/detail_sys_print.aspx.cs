        ...
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                if (sys != null)
                {
                    ...
                    SpecialCashPrice = LU.BLL.FormatProvider.Price(sys.sys_tmp_price.Value * (1 - setting.CardRate));
                    RegularPrice = LU.BLL.FormatProvider.Price(sys.sys_tmp_price.Value);
                }
                ...
                for (int i = 0; i < sysList.Count; i++)
                {
                    ...
                    if (sysList[i].IsCase)
                        CaseImgUrl = "https://lucomputers.com/pro_img/COMPONENTS/" + sysList[i].PartSku + "_list_1.jpg";
                }
                SysListString += "</table>";
            }
        }
