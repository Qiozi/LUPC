...
public partial class cmds_Shopping : PageBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (this.cookiesHelper.CurrOrderCode == 0 && ReqCmd != "getShoppingList")
            {
                Response.Write("Order is not find.");
                Response.End();
            }

            var orderCode = this.cookiesHelper.CurrOrderCode;
                ...
                case "getShoppingList":
                    ...
                    foreach (var tmpM in tmpCartList)
                    {
                        ...
                        if (sku.ToString().Length < 6)
                        {
                            ...
                            if (prod != null)
                            {
                                ...
                                m.ImgUrl =  setting.ImgHost + "pro_img/COMPONENTS/" + (prod.other_product_sku > 0 ? prod.other_product_sku : prod.product_serial_no) + "_list_1.jpg";
                                ...
                                m.PriceUnit = this.cookiesHelper.CurrSiteCountry.ToString();
                                ...
                                SaveToCartTemp(m.SKU
                                    ...
                                    , m.Qty
                                    , this.cookiesHelper.CurrOrderCode);
                                #endregion
                            }
                        }
                        else if (sku.ToString().Length == 8)
                        {
                            ...
                            m.ImgUrl = caseProd == null ? "" : setting.ImgHost +"pro_img/COMPONENTS/" + (caseProd.PartSku) + "_list_1.jpg";
                            m.Title = tmpM.Title;
                            m.SubSold = LU.BLL.PRateProvider.Multiply(m.Sold, m.Qty);
                            m.SubSoldString = LU.BLL.FormatProvider.Price(m.SubSold);
                            m.PriceUnit = this.cookiesHelper.CurrSiteCountry.ToString();
                    ...
                    if (tmpCartList.Count > 0)
                    {
                        ...
                        if (cartPrice == null)
                        {
                            ...
                            cartPrice.price_unit = this.cookiesHelper.CurrSiteCountry.ToString();
                    ...
                    this.cookiesHelper.CartQty = ShopList.Count;
                    //Response.Write(tmpCartList.Count);
                    string json = Newtonsoft.Json.JsonConvert.SerializeObject(ShopList);
                    Response.Write("{\"total\":\"" + LU.BLL.FormatProvider.Price( sub_total) + "\", row:" + json + "}");
                    #endregion
        ...
        , int orderCode)
    {
        ...
        if (cartTemp != null)
        {
            ...
            cartTemp.price_unit = this.cookiesHelper.CurrSiteCountry.ToString();
    ...
    void SaveSubTotal(int orderCode, decimal subTotal)
    {
        ...
        {
            var cp = new LU.Data.tb_cart_temp_price();
            ...
            cp.hst_rate = 0M;
            cp.price_unit = this.cookiesHelper.CurrSiteCountry.ToString();
            cp.pst = 0M;
