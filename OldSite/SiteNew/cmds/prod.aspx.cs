        ...
        if (!IsPostBack)
        {
            switch (ReqCmd)
            {
                case "list":
                    ...
                    #region list
                    var searchPredicate = PredicateBuilder.True<LU.Data.tb_product>();
                    ...
                    {
                        result += ("<div class=\"grid\" onclick=''><a href='/computer/" + (LU.BLL.Util.FilterName(item.manufacturer_part_number)) + "/" + item.product_serial_no + ".html'>");
                        // result += ("<div>");
                        result += (string.Format(@"<div class=""imgholder"">
                            <img class=""lazy""
                                 src='{2}pro_img/ebay_gallery/9/999999_ebay_list_t_1.jpg' 
                                 data-original='{2}pro_img/ebay_gallery/{1}/{0}_ebay_list_t_1.jpg' 
                                width=""135"" alt=""..."">
                         </div>
                        "
                        ...
                        result += ("<p class='prod-long-name'>" + item.product_name_long_en + "</p>");
                        result += ("<div class=\"meta\">");

                        result += "<div class='pull-left'>asdf</div>";
                        if (item.product_current_discount > 0)
                            result += "Save: <small style='color:blue;'>$" + item.product_current_discount + "</small>";
                        result += ("<span class=\"price\">$" + LU.BLL.FormatProvider.Price(item.product_current_price.Value - item.product_current_discount.Value) + "</span>&nbsp;&nbsp;&nbsp;");
                        if (item.product_current_discount > 0)
                            result += "<br>";

                        result += ("<span class=\"text-right\" ><a onclick='util.addToCart(" + item.product_serial_no + ")'><span class='glyphicon glyphicon-shopping-cart'></span> Buy</a></span>");
                        result += ("</div></a>");
                        //result += ("</div>");
                        // result += ("</div>");
                        result += ("</div>");
        ...
        <a class=""btn btn-default {5}"" {4}><span class=""glyphicon glyphicon-chevron-right""></span></a> "
                ...
                case "getPartPriceArea":
                    ...
                    priceListString += string.Format(@"<li class=""list-group-item"">On eBay Price: <span class=""badge "">{0}</span></li>"
                        , ebayPrice == 0M ? "N/A" : "eBay Item id: <a href=\"http://www.ebay.ca/itm/ws/eBayISAPI.dll?ViewItem&Item=" + ebayItemId + "\" target=\"_blank\" style='margin-right:2em;'>" + ebayItemId + "</a><span style='color:blue;'>$" + LU.BLL.FormatProvider.Price(ebayPrice) + "</span> <small>" + ebayPriceUnit + "</small>");
                ...
                case "getSinglePrice":
                    bool writed = false;
                    string priceFile = Server.MapPath("~/Computer/part_price/" + ReqSku + "_" + this.cookiesHelper.CurrSiteCountry.ToString() + ".txt");
                    ...
                    if (!writed)
                    {
                        ...
                        {
                            string priceStr = "{price:'" + ConvertPrice(prodList2[0].Price - prodList2[0].Save) + "',save:'" + ConvertPrice(prodList2[0].Save) + "',unit:'" + (this.cookiesHelper.CurrSiteCountry == CountryType.CAD ? "CAD" : "USD") + "'}";
