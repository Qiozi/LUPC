...
"
            ...
            foreach (var ct in cartTemp)
            {
                ...
                prodListString += "<tr " + (haveSysProd ? " class='success'" : "") + "><td>" + logostr + "</td><td>" + ct.product_name + "</td><td width='50'>" + ct.order_product_sum + "</td><td width='80' class='text-right'>$" + ct.order_product_price + "</td><td width='90' class='text-right'>$" + LU.BLL.FormatProvider.Price(ct.order_product_price.Value * ct.order_product_sum.Value) + "</td></tr>";
                if (ct.product_serial_no.Value.ToString().Length == 8)
                {
                    #region 系统列表明细
           ...
           // if (tempPrice != null)
            {
                ...
                tax = (LU.BLL.PRateProvider.Multiply(taxable_total, oh.pst_rate.Value / 100))
                    + (LU.BLL.PRateProvider.Multiply(taxable_total, oh.gst_rate.Value / 100))
                    + (LU.BLL.PRateProvider.Multiply(taxable_total, oh.hst_rate.Value / 100));
            ...
            <div class=""row"">
                ...
                , LU.BLL.FormatProvider.Price(ConvertPrice(subTotal))
                , LU.BLL.FormatProvider.Price(ConvertPrice(shippingAndHandling))
                , GetTaxString(oh, ConvertPrice(taxable_total))
                , LU.BLL.FormatProvider.Price(ConvertPrice(grand_total))
                , oh != null ? oh.price_unit : ""
                , LU.BLL.FormatProvider.Price(ConvertPrice(taxable_total))
                , LU.BLL.FormatProvider.Price(ConvertPrice(specialCashDiscount))
                );
    ...
    /// <param name="cp"></param>
    /// <returns></returns>
    string GetTaxString(LU.Data.tb_order_helper cp, decimal taxable_total)
    {
        if (cp == null)
            return "";
        if (cp.hst_rate > 0)
        {
            return @"<div class=""row"">
                ...
                <div class=""col-sm-3 text-right price"">$" + LU.BLL.PRateProvider.Multiply(taxable_total, cp.hst_rate.Value / 100) + @" <small>" + cp.price_unit + @"</small></div>
            </div>";
        }
        ...
        if (cp.gst_rate > 0)
        {
            result += @"<div class=""row"">
                        ...
                        <div class=""col-sm-3 text-right price"">$" + LU.BLL.PRateProvider.Multiply(taxable_total, cp.gst_rate.Value / 100) + @" <small>" + cp.price_unit + @"</small></div>
                    </div>";
        }
        ...
        if (cp.pst_rate > 0)
        {
            result += @"<div class=""row"">
                        ...
                        <div class=""col-sm-3 text-right price"">$" + LU.BLL.PRateProvider.Multiply(taxable_total, cp.pst_rate.Value / 100) + @" <small>" + cp.price_unit + @"</small></div>
                </div>";
        }
