...
public partial class MyAccount : PageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (IsLogin)
            {
                var userId = LU.BLL.Users.UserToken.GetUserId(db, cookiesHelper.UserToken);
                var cust = db.tb_customer.FirstOrDefault(p => p.ID.Equals(userId));
                if (cust != null)
                {
    ...
    string GetStateOptionString(int StateID, string StateCode, List<LU.Data.tb_state_shipping> list)
    {
