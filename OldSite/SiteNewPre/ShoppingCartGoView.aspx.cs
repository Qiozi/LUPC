...
public partial class ShoppingCartGoView : PageBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            var cartTemp = db.tb_cart_temp.Where(p => p.cart_temp_code.HasValue
                && p.cart_temp_code.Value.Equals(CurrOrderCode)).ToList();
            ...
            foreach (var ct in cartTemp)
            {
                ...
                prodListString += "<tr " + (haveSysProd ? " class='success'" : "") + "><td>" + logostr + "</td><td>" + ct.product_name + "</td><td width='50'>" + ct.cart_temp_Quantity + "</td><td width='80' class='text-right'>$" + ct.price + "</td><td width='90' class='text-right'>$" + PriceRate.Format(ct.price.Value * ct.cart_temp_Quantity.Value) + "</td></tr>";
                if (ct.product_serial_no.Value.ToString().Length == 8)
                {
                    #region 系统列表明细
                ...
                else
                {
                        ...
                        && ct.shipping_company.Value > 0)
                    {
                        ...
                        if (shipComp != null)
                        {
                            ...
                            int csn;
                            int.TryParse(CustomerSerialNo, out csn);
                            var customer = db.tb_customer.FirstOrDefault(p => p.customer_serial_no.HasValue
                                && p.customer_serial_no.Value.Equals(csn));
            ...
            if (tempPrice != null)
            {
                ...
                tax = (PriceRate.Multiply(taxable_total, tempPrice.pst_rate.Value / 100))
                    + (PriceRate.Multiply(taxable_total, tempPrice.gst_rate.Value / 100))
                    + (PriceRate.Multiply(taxable_total, tempPrice.hst_rate.Value / 100));
            ...
            <div class=""row"">
                ...
                , PriceRate.Format((subTotal))
                , PriceRate.Format((shippingAndHandling))
                , GetTaxString(tempPrice, (taxable_total))
                , PriceRate.Format((grand_total))
                , tempPrice != null ? tempPrice.price_unit : ""
                , PriceRate.Format((taxable_total))
                , PriceRate.Format((specialCashDiscount))
                );
            ...
            if (IsToPaypal)
            {
                ...
                Session["paypalOrderCode"] = CurrOrderCode.ToString();
            ...
            if (paymentCardDoDirect == 25)
            {
                ...
                btnNext.Attributes.Remove("class");
                btnNext.InnerHtml = "Submit Payment <img src='https://www.paypalobjects.com/webstatic/en_US/btn/btn_buynow_pp_142x27.png' border='0' align='top' alt='Buy Now with PayPal'/>";
            }
        }
    }
    /// <summary>
    ...
    /// <param name="cp"></param>
    /// <returns></returns>
    string GetTaxString(nicklu2Model.tb_cart_temp_price cp, decimal taxable_total)
    {
        if (cp == null)
            return "";
        if (cp.hst_rate > 0)
        {
            return @"<div class=""row"">
                ...
                <div class=""col-md-3 text-right price"">$" + PriceRate.Multiply(taxable_total, cp.hst_rate.Value / 100) + @" <small>" + cp.price_unit + @"</small></div>
            </div>";
        }
        ...
        if (cp.gst_rate > 0)
        {
            result += @"<div class=""row"">
                        ...
                        <div class=""col-md-3 text-right price"">$" + PriceRate.Multiply(taxable_total, cp.gst_rate.Value / 100) + @" <small>" + cp.price_unit + @"</small></div>
                    </div>";
        }
        ...
        if (cp.pst_rate > 0)
        {
            result += @"<div class=""row"">
                        ...
                        <div class=""col-md-3 text-right price"">$" + PriceRate.Multiply(taxable_total, cp.pst_rate.Value / 100) + @" <small>" + cp.price_unit + @"</small></div>
                </div>";
        }
