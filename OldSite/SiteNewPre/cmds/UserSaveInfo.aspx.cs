    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            switch (cmd)
            {
                case "savePickup":
                    #region savePickup
                    var customSerial = int.Parse(CustomerSerialNo);
                    var customer = db.tb_customer.FirstOrDefault(p => p.customer_serial_no.HasValue
                        && p.customer_serial_no.Value.Equals(customSerial));
                    if (customer != null)
                    {
                        ...
                        customer.pay_method = 21;
                        string pickupDate = Util.GetStringSafeFromString(Page, "pickupDate");
                        string pickupTime = Util.GetStringSafeFromString(Page, "pickupTime");
                        ...
                        if (pickupDate != "0")
                        {
                            ...
                            DateTime.TryParse(pickdt.ToString("yyyy-MM-dd") + " " + pickupTime + ":00", out pickdt);
                        ...
                        OrderHelper.SaveOrderNote(Util.GetStringSafeFromString(Page, "comment"), CurrOrderCode, true, db);
                ...
                case "saveEmailTransfer":
                    ...
                    customSerial = int.Parse(CustomerSerialNo);
                    customer = db.tb_customer.FirstOrDefault(p => p.customer_serial_no.HasValue
                     && p.customer_serial_no.Value.Equals(customSerial));
                    ...
                    if (customer != null)
                    {
                        ...
                        ChangeTaxRate(statid, CurrOrderCode.ToString());
                ...
                case "saveMyAccount":
                    ...
                    if (customer != null)
                    {
                        ...
                        ChangeTaxRate(StatID, CurrOrderCode.ToString());     // 变化订单 收税的洲值             
                ...
                case "saveCreditCard":
                    ...
                    if (customer != null)
                    {
                        ...
                        OrderHelper.SaveOrderNote(Util.GetStringSafeFromString(Page, "notes"), CurrOrderCode, true, db);
