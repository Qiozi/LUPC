...
public partial class cmds_systemProd : PageBase
{
    ...
    void WriteSysPrice(int sysSku, bool isFormat)
    {
        ...
        Response.Write(string.Format(@"{{""Price"":""{0}"",""Discount"":""{1}"",""Sold"":""{2}"", ""Unit"":""{3}""}}"
            , isFormat ? PriceRate.Format(ConvertPrice(sysPartList.Sum(p => p.PartPrice))) : ConvertPrice(sysPartList.Sum(p => p.PartPrice)).ToString()
            , isFormat ? PriceRate.Format(ConvertPrice(sysPartList.Sum(p => p.PartDiscount))) : ConvertPrice(sysPartList.Sum(p => p.PartDiscount)).ToString()
            , isFormat ? PriceRate.Format(ConvertPrice(sysPartList.Sum(p => p.PartPrice - p.PartDiscount))) : ConvertPrice(sysPartList.Sum(p => p.PartPrice - p.PartDiscount)).ToString()
...
>
    ...
    <span class='note itemPrice'>${2}</span>
               ...
               , p.Title
               , "https://lucomputers.com/pro_img/COMPONENTS/" + p.ImgSku + "_t.jpg"
               , p.Sold
