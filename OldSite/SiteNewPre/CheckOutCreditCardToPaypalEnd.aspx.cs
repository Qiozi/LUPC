    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack && IsLocalHostFrom && IsLogin)
        {
            ...
            if (!string.IsNullOrEmpty(TRANSACTIONID) && OC > 0)
            {
                ...
                var orderPayRecord = new nicklu2Model.tb_order_pay_record();
                ...
                db.AddTotb_order_pay_record(orderPayRecord);
                // order paypal record
                var orderPaypal = new nicklu2Model.tb_order_paypal_record();
                orderPaypal.transaction = TRANSACTIONID;
                orderPaypal.avs = AVS;
                orderPaypal.cvv2 = CVV2;
                orderPaypal.order_code = OC;
                orderPaypal.regdate = DateTime.Now;
                db.AddTotb_order_paypal_record(orderPaypal);
                // order
