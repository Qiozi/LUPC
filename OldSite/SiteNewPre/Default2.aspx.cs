...
public partial class Default2 : PageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                index++;
                cateString += "<li><a href='#c" + c.Value + "' data-toggle='tooltip' data-placement='top' title='" + c.Text + "'>" + c.Text + "</a></li>";
                cateListStr += "<div id='c" + c.Value.ToString() + "' class='cateTitle'><h3> <a href='list_part.aspx?cid=" + c.Value + "'><span class='glyphicon glyphicon-flag'></span> " + c.Text + "<small>&nbsp;Press for more</small></a></h3></div>";
                cateListStr += "<div class='row catelist' style='padding:5px;' cateid='" + c.Value + "'>";
                cateListStr += "</div>";
            }
        ...
        for (int i = 0; i < sysSkus.Count; i++)
        {
            ...
            foreach (var sysPart in sysList1)
            {
                ...
                if (sysPart.PartSku != 16684 && !string.IsNullOrEmpty(commentName) && !string.IsNullOrEmpty(sysPart.ShortName))
                {
                    ...
                    sysResult += "<a class='list-group-item col-md-9' style='color:#333;'>" + sysPart.ShortName + "</a>";
