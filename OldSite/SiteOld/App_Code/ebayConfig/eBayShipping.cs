...
public class eBayShipping
{
    ...
    /// <returns></returns>
    public static string GetShippingDetail2(
        bool is_notebook
        , int sku
        , bool IsSystem
        , bool is_shrink
...
"
     ...
     , is_notebook && international_service_3 == "CA_UPSWorldWideExpedited" ? GetNotebookShipToCountry(scanSize, weight, n , "CA_UPSWorldWideExpedited", sku) : ""));
                n += 2;
            }
            #endregion
        }
...
", domestic_free_shipping
    ...
    /// <returns></returns>
    public static string GetNotebookShipToCountry(decimal scanSize, decimal weight, int priority, string shippingCompany, int sku)
    {
        var pm = ProductModel.GetProductModel(sku);
        decimal minShipping = GetEbayPrice.GetPartMinShippingFee(pm);
...
"
    ...
    /// <param name="pm"></param>
    /// <returns></returns>
    public static string GetPartShippingFeeString(ProductModel pm)
    {
                            ...
                            sb.Append(string.Format(@"
                              <InternationalShippingServiceOption>
                                ...
                                <ShipToLocation>US</ShipToLocation>
                                          ...
                                          , list[i].IsFree ? "0" : (list[i].shippingFee >= minShipping ? (list[i].shippingFee - minShipping).ToString() : list[i].shippingFee.ToString())
                                          , list[i].IsFree ? "0" : new eBayPriceHelper().eachAddItemShipping(list[i].shippingFee, minShipping).ToString()
                                          , (list[i].IsFree || (list[i].shippingFee >= minShipping ? (list[i].shippingFee - minShipping) : list[i].shippingFee) == 0M) ? "<FreeShipping>true</FreeShipping>" : ""));
                        }
