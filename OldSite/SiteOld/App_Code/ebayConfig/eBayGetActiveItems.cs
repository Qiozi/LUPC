...
update tb_ebay_system_parts set is_online=1 where system_sku={0};", dr["sys_sku"].ToString()));
        ...
        Config.ExecuteNonQuery(@"
insert into tb_ebay_code_and_luc_sku (sku, is_sys, ebay_code, is_online)
select luc_sku, 0, itemid, 1 from tb_ebay_selling where luc_sku>0 and itemid not in (select ebay_code from tb_ebay_code_and_luc_sku);
");
        Config.ExecuteNonQuery(@"
insert into tb_ebay_code_and_luc_sku (sku, is_sys, ebay_code, is_online)
select sys_sku, 1, itemid, 1 from tb_ebay_selling where sys_sku>0 and itemid not in (select ebay_code from tb_ebay_code_and_luc_sku);
