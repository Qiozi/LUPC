...
{
    OrderHelperModel OH = null;
    CustomerStoreModel CS = null;

    string _OrderCode = "";
   
    public OrderPriceWork(int OrderCode)
	{
        _OrderCode = OrderCode.ToString();

        OH = OrderHelperModel.GetModelByOrderCode(OrderCode);
        CS = CustomerStoreModel.FindByOrderCode(OrderCode.ToString());
    ...
    void InitTax(StateShippingModel state)
    {
        if (!OH.is_lock_tax_change)
        {
            OH.hst_rate = 0M;
            OH.gst_rate = 0M;
            OH.pst_rate = 0M;
    ...
    public void AccountOrder(string order_code, bool is_save)
    {
        ...
        //
        // 得到洲ID
        int stateid = CS.customer_shipping_state;
        if (stateid < 1)
        {
            stateid = StateShippingModel.FindStatIDByCode(CS.shipping_state_code);
            if (stateid > 1)
            {
...
select 	prod_shipping_fee_id, prod_Sku, is_system, shipping_fee_us, shipping_fee_ca, regdate
	...
	where prod_sku='{0}' and is_system='{1}'", dr["product_serial_no"].ToString(), 0));
                    ...
                    if (sales_promotion_dt.Rows.Count == 0 || Config.sale_promotion_compay_id.IndexOf("[" + OH.shipping_company + "]") == -1)
                    {
                        ...
                        model.shipping_company_id = OH.shipping_company;
            ...
            for (int i = 0; i < noebookDT.Rows.Count; i++)
            {
                ...
                for (int x = 0; x < product_sum; x++)
                {
                    int product_id = int.Parse(dr["product_serial_no"].ToString());
                    ProductModel p = ProductModel.GetProductModel(product_id);
                    _cost += p.product_current_cost * 1;
...
select 	prod_shipping_fee_id, prod_Sku, is_system, shipping_fee_us, shipping_fee_ca, regdate
	...
	where prod_sku='{0}' and is_system='{1}'", product_id, 0));
        ...
        try
        {
            ...
            {
                if (ap_real_count > 0 && !OH.is_lock_shipping_charge)
                {
...
//        ,weee_charge from tb_order_helper where order_code='" + order_code + "'");
        if (OH !=null)
        {
            ...
            is_lock_input_order_discount = OH.is_lock_input_order_discount;
            is_lock_shipping_charge = OH.is_lock_shipping_charge;

            if (is_lock_input_order_discount == true)
                input_order_discount = OH.input_order_discount;
            if (is_lock_shipping_charge)
                shipping_charge = OH.shipping_charge;
            else
                shipping_charge = _result;

           weee_charge = OH.weee_charge;
        ...
        SaveToDB(stateid, OH.shipping_company, _price_sum, _total, _result, _cost, _sale_tax
        ...
        , decimal weee_charge)
    {
        try
        {
            ...
            _sale_tax = OH.gst + OH.pst + OH.hst;
