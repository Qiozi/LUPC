...
{
    int current_sku = 0;
    string source_pathfile = "/pro_img/source_components/";
    string part_pathfile = "~/pro_img/COMPONENTS/";
    string part_pathfile_no_shuiying = "~/pro_img/not_shui_yin/";
    string image_url = "/pro_img/COMPONENTS/";//"http://www.lucomputers.com/pro_img/COMPONENTS/";
    string ebay_path = "~/pro_img/ebay_gallery/";
    ...
    {
        string webfilename = Server.MapPath(source_pathfile + sku.ToString() + ".jpg");
    ...
    private void SaveUpload(FileUpload FileUpload1, int index, bool IsUpload, string webfilename)
    {
        ...
        else
        {
            if (FileUpload1.HasFile)
            {
                ...
                webFilePath = Server.MapPath(source_pathfile + fileName);        // 服务器端文件路径
        ...
        string webFilePath_s = Server.MapPath("~/txts/" + fileName_s);　　// 服务器端缩略图路径
        ...
        //if (!File.Exists(webFilePath))
        {
            try
            {
                ...
                string min_image_have_shuiyin = Server.MapPath(string.Format("{1}{0}", min_image, part_pathfile));
                string list_image_have_shuiyin = Server.MapPath(string.Format("{1}{0}", list_image, part_pathfile));
                string max_image_have_shuiyin = Server.MapPath(string.Format("{1}{0}", max_image, part_pathfile));
    ...
    private string GenerateNoShuiYinPath(int sku)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            string path = Server.MapPath(string.Format("{0}{1}/", part_pathfile_no_shuiying, dt.Rows[0][0].ToString()));
    ...
    {
        string _path = Server.MapPath(string.Format("{0}{1}", path, filename.Trim().Substring(0, 1)));
    ...
    {
        DirectoryInfo dir = new DirectoryInfo(Server.MapPath(part_pathfile));
        ...
        if (count > 0)
        {
           // throw new Exception(sku.ToString());
            ...
            pm.product_img_sum = byte.Parse(count.ToString());
            pm.Update();
        }
    ...
    protected void btn_generate_photo_for_ebay_Click(object sender, EventArgs e)
    {
        ...
        for(int i=0; i<fis.Length; i++)
        {
            if (fis[i].Name.IndexOf("_list_1.jpg") != -1)
            {
                ...
                MakeThumbnail(fis[i].FullName, Server.MapPath(part_pathfile + "_t_ebay_flash_1.jpg"), 210, 210, "Cut");
                MakeThumbnail(fis[i].FullName, Server.MapPath(part_pathfile+"_t_ebay_info_1.jpg"), 75, 75, "Cut");
                MakeThumbnail(fis[i].FullName, Server.MapPath(part_pathfile + "_g_ebay_info_1.jpg"), 350, 350, "Cut");
    ...
    {
        string imgSkuString = Server.MapPath(string.Format("{0}\\{1}_list_1.jpg", part_pathfile, sku));

        if (File.Exists(imgSkuString))
        {
            MakeThumbnail(imgSkuString, Server.MapPath(part_pathfile + "_t_ebay_flash_1.jpg"), 210, 210, "Cut");
        }
    ...
    protected void btn_tmp_Click(object sender, EventArgs e)
    {
        if (this.CategoryDropDownLoad1.categoryId > 0)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                string simall_file = Server.MapPath(string.Format("{0}{1}_t.jpg", path, dr["product_serial_no"].ToString()));
                string source_file= Server.MapPath(string.Format("{0}{1}_t.jpg", part_pathfile, dr["product_serial_no"].ToString()));
                if (!File.Exists(simall_file))
                {
                    ...
                    for (int j = 0; j < sum; j++)
                    {
                        string list_file = Server.MapPath(string.Format("{0}{1}_list_{2}.jpg", path, dr["product_serial_no"].ToString(), j + 1));
                        string big_file = Server.MapPath(string.Format("{0}{1}_g_{2}.jpg", path, dr["product_serial_no"].ToString(), j + 1));

                        string source_list_file = Server.MapPath(string.Format("{0}{1}_list_{2}.jpg", part_pathfile, dr["product_serial_no"].ToString(), j + 1));
                        string source_big_file = Server.MapPath(string.Format("{0}{1}_g_{2}.jpg", part_pathfile, dr["product_serial_no"].ToString(), j + 1));
