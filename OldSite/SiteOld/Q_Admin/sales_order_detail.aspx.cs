...
public partial class Q_Admin_sales_order_detail : PageBase
{
    ...
    private void SetControls(string order_code)
    {
        ...
        OrderHelperModel[] model = OrderHelperModel.GetModelsByOrderCode(int.Parse(order_code));
        CustomerStoreModel[] css = CustomerStoreModel.FindModelsByOrderCode(order_code);

        if (model.Length == 1 && css.Length == 1)
        {
            OrderHelperModel orderHelper = model[0];
            CustomerStoreModel customer = css[0];
            StateShippingModel state = StateShippingModel.GetStateShippingModel(customer.customer_shipping_state);
            ...
            this.lbl_grand_total.Text = string.Format("{0}{1}", Config.ConvertPrice(orderHelper.grand_total), price_unit_string);
            ...
            if (pay_method_id != 0)
                this.lbl_payment.Text = PayMethodNewModel.GetPayMethodNewModel(pay_method_id).pay_method_short_name;
            if (Config.pay_method_pick_up_ids.IndexOf("[" + pay_method_id.ToString() + "]") != -1)// == Config.pay_method_pick_up_id)
            {
                this.lbl_prick_time.Text = "<table cellspacing='0' cellpadding='0'><tr><td><strong><span style=\"font-size: 9pt\">Pick Up Schedule:</strong></span></td><td>" + (orderHelper.prick_up_datetime1.Year == 1 || (orderHelper.prick_up_datetime1.Month == 1 && orderHelper.prick_up_datetime1.Day == 1 && orderHelper.prick_up_datetime1.Hour == 11) ? "" : string.Format("{0:t},{0:D}", orderHelper.prick_up_datetime1)) + "</td></tr><tr><td>&nbsp;</td><td>" + (orderHelper.prick_up_datetime2.Year == 1 || (orderHelper.prick_up_datetime2.Month == 1 && orderHelper.prick_up_datetime2.Day == 1 && orderHelper.prick_up_datetime2.Hour == 11) ? "" : string.Format("{0:t},{0:D}", orderHelper.prick_up_datetime2)) + "</td></tr></table>";
                if (orderHelper.shipping_charge == 0M)
                    this.lbl_shipping_and_handling.Text = "$0.00" + price_unit_string;
                else
                {
                    ShippingCompanyModel scm = ShippingCompanyModel.GetShippingCompanyModel(orderHelper.shipping_company);
                    this.lbl_shipping_company.Text = scm != null ? scm.shipping_company_name : "";


                    this.lbl_shipping_and_handling.Text = string.Format("{0}{1}", Config.ConvertPrice(orderHelper.shipping_charge), price_unit_string);
            ...
            this.lbl_special_cash_discount.Text = string.Format("{0}{1}", orderHelper.input_order_discount.ToString("$0.##"), price_unit_string);
            ...
            if (customer.shipping_state_code.Trim().ToLower() == "BRITISH COLUMBIA".ToLower() || customer.shipping_state_code == "BC")
            {
                ...
                {
                    tax_string += string.Format("<b>GST({0}%)</b></td><td style='text-align:right;'>{1}{2}</td>", orderHelper.gst_rate.ToString("##"), Config.ConvertPrice(orderHelper.gst), price_unit_string);
                }
                if (orderHelper.pst_rate > 0M)
                {
                    tax_string += string.Format("</tr><tr><td><b>PST-BC({0}%)</b></td><td style='text-align:right;'>{1}{2}</td>", orderHelper.pst_rate.ToString("##"), Config.ConvertPrice(orderHelper.pst), price_unit_string);
                }
                if (orderHelper.hst_rate > 0M)
                {
                    var gstRate = 5;
                    var pstRate = orderHelper.hst_rate - gstRate;
                    var gst = decimal.Parse((orderHelper.taxable_total * gstRate / 100).ToString("0.00"));
                    ...
                    tax_string += string.Format("</tr><tr><td style='text-align:right;'><b>PST-BC({0}%)</b></td><td style='text-align:right;'>{1}{2}</td>"
                        , pstRate.ToString("##")
                        , Config.ConvertPrice(orderHelper.hst - gst)
                        , price_unit_string);
                }
            }
            else
            {
                if (orderHelper.hst_rate > 0M)
                {
                    tax_string += string.Format("<b>HST({0}%)</b></td><td style='text-align:right;'>{1}{2}</td>", orderHelper.hst_rate.ToString("##"), Config.ConvertPrice(orderHelper.hst), price_unit_string);
                }
                else
                {
                    if (orderHelper.gst_rate > 0M)
                    ...
                    if (orderHelper.pst_rate > 0M)
                    {
                        tax_string += string.Format("</tr><tr><td style='text-align:right;'><b>PST({0}%)</b></td><td style='text-align:right;'>{1}{2}</td>", orderHelper.pst_rate.ToString("##"), Config.ConvertPrice(orderHelper.pst), price_unit_string);
                    }
                }
                if ((orderHelper.pst_rate == 0M &&
                ...
                {
                    tax_string = string.Format("<b>Sales Tax</b></td><td style='text-align:right;'>{0}{1}</td>", Config.ConvertPrice(orderHelper.gst + orderHelper.pst), price_unit_string);
                }
            }
            #endregion
            // tax
            this.lbl_tax_rate.Text = tax_string;
            this.lbl_sub_total.Text = string.Format("{0}{1}", Config.ConvertPrice(orderHelper.sub_total_rate), price_unit_string);
            ...
            if (orderHelper.is_old == 1)
            {
                this.lbl_is_old_order.Text = "This is Old Order.";
            }
            this.lbl_taxable_total.Text = string.Format("{0}{1}", orderHelper.taxable_total.ToString("$0.00"), price_unit_string);
            ...
            OrderEbayModel[] oem = OrderEbayModel.FindAllByProperty("order_code", ReqOrderCode);
...
", oem[0].shipping_service
 , oem[0].item_number);
    ...
    private void GetOrderListString(string order_code)
    {
        ...
        OrderProductModel[] opm = OrderProductModel.GetModelsByOrderCode(order_code);
        ...
        for (int i = 0; i < opm.Length; i++)
        {
            ...
            if (opm[i].product_serial_no.ToString().Length == 8)
                ...
                sb.Append("<td >" + opm[i].product_name + "(<a href=\"editPartDetail.aspx?id=" + opm[i].product_serial_no.ToString() + "\" onclick=\"winOpen(this.href, 'right_m',880,800,120,200);return false;\"><span style='color:#000000;'>" + opm[i].product_serial_no.ToString() + "</span></a>)" + (opm[i].product_serial_no.ToString().Length == 8 ? "" : pm.FindStockByLuSkuForOrder(opm[i].product_serial_no, -1)) + "</td>  ");
            sb.Append("<td width=\"9%\" align=\"center\">" + opm[i].order_product_sum.ToString() + "</td> ");
            sb.Append("<td width=\"11%\" align=\"right\" >" + Config.ConvertPrice(opm[i].order_product_sold) + "</td> ");
            sb.Append("<td width=\"9%\" align=\"right\" >" + Config.ConvertPrice(opm[i].order_product_sold * opm[i].order_product_sum) + "</td>  ");
            sb.Append("</tr>");
...
where product_serial_no='" + sdm.Rows[j]["product_serial_no"].ToString() + "'");
 ...
 on oi.other_inc_sku=ol.other_inc_sku and ol.other_inc_type=oi.other_inc_id and ol.lu_sku='{0}' and tag=1", sdm.Rows[j]["product_serial_no"].ToString()));
    ...
    private void BindOrderProductHistory(string order_code)
    {
        ...
        if (show_history)
        {
            ophm = OrderProductHistoryModel.FindModelsByOrder(order_code);
        }
    ...
    protected void ddl_order_status_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (orderCode > 0)
        {
            ...
            new OrderHelperModel().UpdateOutStatus(-1, intValue, "", OrderHelperID, -1);
            this.InsertTraceInfo("Save Order Note And Status:" + ReqOrderCode.ToString());
