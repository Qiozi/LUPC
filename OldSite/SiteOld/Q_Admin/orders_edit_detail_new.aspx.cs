...
public partial class Q_Admin_orders_edit_detail_new : PageBase
{
    ...
    void InitPage()
    {
        if (ReqOrderCode>0)
        {
            ...
            CS = CustomerStoreModel.FindByOrderCode(ReqOrderCode.ToString());
            if (OH == null
            || CS == null)
            {
    ...
    void ViewPrice()
    {
        ...
        {
            PriceInfo.ShipCharge = OH.shipping_charge;// ConvertPrice.RoundPrice(dr["shipping_charge"].ToString());
            // _priceInfo this.lbl_order_code.Text = order_code;
            PriceInfo.GrandTotal = OH.grand_total;// this.lbl_grand_total.Text = ConvertPrice.RoundPrice(dr["grand_total"].ToString());
            PriceInfo.SubTotal = OH.sub_total;//  this.lbl_sub_total.Text = ConvertPrice.RoundPrice(dr["sub_total"].ToString());
            PriceInfo.SpecialCashDiscount = OH.input_order_discount;//  this.lbl_special_cash_discount.Text = ConvertPrice.RoundPrice(dr["input_order_discount"].ToString());
            PriceInfo.InputDiscount = OH.input_order_discount;//
            PriceInfo.TaxableTotal = OH.taxable_total;// this.lbl_taxable_total.Text = ConvertPrice.RoundPrice(dr["taxable_total"].ToString());
            PriceInfo.Weee = OH.weee_charge;// this.lbl_weee.Text = ConvertPrice.RoundPrice(dr["weee_charge"].ToString());
            PriceInfo.PriceUnit = OH.price_unit;
            PriceInfo.Hst = OH.hst;
            PriceInfo.Hst_rate = OH.hst_rate;
            PriceInfo.Gst = OH.gst;
            PriceInfo.Gst_rate = OH.gst_rate;
            PriceInfo.Pst = OH.pst;
            PriceInfo.Pst_rate = OH.pst_rate;
            //decimal pst_rate;
            ...
            //if (OH.hst_rate > 0M)
            //{
            //    _priceInfo.TaxPercent = OH.hst_rate;
            //    _priceInfo.TaxableTotal = OH.hst;
            ...
            //            _priceInfo.TaxPercent = OH.gst_rate;
            //        _priceInfo.TaxableTotal = OH.gst;
            //    }
            //    if (OH.pst_rate > 0M)
            //    {
            //        if (OH.gst_rate > 0M)
    ...
    public string ViewOtherInfo
    {
        get
        {
            ...
            ShippingCompanyModel scm = ShippingCompanyModel.GetShippingCompanyModel(OH.shipping_company);
            if (scm != null)
                shipCompanyName = scm.shipping_company_name;

            string preStatusName = "";
            PreStatusModel psm = PreStatusModel.GetPreStatusModel(OH.pre_status_serial_no);
            if (psm != null) 
            preStatusName = psm.pre_status_name;

            string backStatusName = "";
            FactureStateModel fsm = FactureStateModel.GetFactureStateModel(OH.out_status);
            if (fsm != null)
...
<tr>
    ...
    <td class='title'><td></td>
            ...
            "
             ...
             , CS.pay_method < 1 ? "" : PayMethodNewModel.GetPayMethodNewModel(CS.pay_method).pay_method_name.ToString()
             , OH.prick_up_datetime1.Year == 1 ||(OH.prick_up_datetime1.Month == 1 && OH.prick_up_datetime1.Day == 1 && OH.prick_up_datetime1.Hour ==11) ? "" : Config.pay_method_pick_up_ids.IndexOf("[" + CS.pay_method.ToString() + "]") > -1 ? string.Format("{0:t},{0:D}", OH.prick_up_datetime1) : ""
             , string.Format("{0} ({1}) {2}",shipCompanyName , OH.shipping_charge.ToString("$0.00"), OH.is_lock_shipping_charge?"<span style='color:blue;'>Locked</span>":"")
             , preStatusName
              );
            return str;
        }
...
"
                ...
                , Tr2("Special Cash Discount", ConvertPrice.RoundPrice(PriceInfo.SpecialCashDiscount).ToString(), (OH.is_lock_input_order_discount ? "<span title='Locked' style='color:blue'>L</span>" : ""), BtnInputDiscount)
                , Tr2("Ship Charge", ConvertPrice.RoundPrice(PriceInfo.ShipCharge).ToString(), (OH.is_lock_shipping_charge ? "<span title='Locked' style='color:blue'>L</span>" : ""), BtnInputShipCharge)
...
>Modify</a>");
        ...
        try
        {
            ...
            //if (!SetSystemProduct(_product_id))
            {
                ...
                else
                {
                    ...
                    order.order_product_cost = ConvertPrice.Price(OH.price_unit.ToLower().ToString() == "cad" ? CountryCategory.CA : CountryCategory.US, product.product_current_cost);
                    order.order_product_price = ConvertPrice.Price(OH.price_unit.ToLower().ToString() == "cad" ? CountryCategory.CA : CountryCategory.US, product.product_current_price);
                    ...
                    order.order_product_sold = ConvertPrice.Price(OH.price_unit.ToLower().ToString() == "cad" ? CountryCategory.CA : CountryCategory.US, product.product_current_price - product.product_current_discount);// ProductModel.FindOnSaleDiscountByPID(_product_id);
                    ...
                    order.product_current_price_rate = ConvertPrice.Price(OH.price_unit.ToLower().ToString() == "cad" ? CountryCategory.CA : CountryCategory.US, product.product_current_price);
                    order.prodType = product.prodType;
                    order.Create();

                    CH.Alert(KeyFields.SaveIsOK, this.lv_part_list);
                    InsertTraceInfo(string.Format("insert part({0}) in order({1})", _product_id, ReqOrderCode.ToString()));
                    //
    ...
    protected void btn_Add_Part_Name_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            InsertTraceInfo("Insert new part to IssueStore (in order[" + ReqOrderCode.ToString() + "]): " + pm.product_serial_no.ToString());
            ...
            InsertTraceInfo(string.Format("insert part({0}) in order({1})", _product_id, ReqOrderCode));
            //
            //  if the order is OK then save a product after create.
            //
            if (OH.is_ok)
    ...
    protected void btn_add_sys_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            InsertTraceInfo(string.Format("add system{1} to order({0})", ReqOrderCode, system_tmp_sku));
            CH.CloseParentWatting(this.lv_sys_list);
    ...
    protected void btn_duplicate_sys_Click(object sender, EventArgs e)
    {
        try
        {
           ...
           // throw new Exception(system_tmp_sku);
            ...
            InsertTraceInfo(string.Format("Duplicate system{1} to order({0})", ReqOrderCode, system_tmp_sku));
            CH.CloseParentWatting(this.lv_sys_list);
            ...
            switch (e.CommandName)
            {
                ...
                case "AddPartToSys":
                   ...
                   // throw new Exception((_sold + ((p.product_current_price - p.product_current_discount) * part_quantity)).ToString());
                    ...
                    ChangeSystemPrice(e.Item, _system_code, OH.order_source == 3, _sold + ((p.product_current_price - p.product_current_discount) * part_quantity));
                ...
                case "DeleteSys":
                    ...
                    InsertTraceInfo(string.Format("delete to system({1}) in order({0})", ReqOrderCode, _serial_no));
                    break;
            }
            OrdersSavePageRedirect();
            // CH.Alert("OK", this.lv_sys_list);
                ...
                case "DelSysDetail":
                    ...
                    for (int i = 0; i < this.lv_sys_list.Items.Count; i++)
                    {
                        ...
                        if (rpt == _rpt)
                        {
                            ...
                            {
                                OrderProductModel[] ops = OrderProductModel.GetModelsByProductCode(system_code);
                                orderProductPrice = ops[0].order_product_sold;
                            }
                            ChangeSystemPrice(this.lv_sys_list.Items[i], system_code, OH.order_source == 3, orderProductPrice - (pm.product_current_price - pm.product_current_discount) * part_quantity);
                        }
                    }
                    break;
            }
    ...
    private void SetProductToSystemDetail(int system_code, int product_id, int part_quantity)
    {
        ...
        InsertTraceInfo(string.Format("add part({2}) to system({1}) in order({0})", ReqOrderCode, system_code, product_id));
...
select order_product_price, order_product_cost, order_product_sold, save_price, product_current_price_rate from tb_order_product where  product_serial_no='{0}';", system_code));
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            if (!isEbay)
            {
                OrderProductModel opm = OrderProductModel.GetOrderProductModel(int.Parse(_hf_serial_no.Value));
