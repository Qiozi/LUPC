...
group by p.menu_child_serial_no");
    ...
    private void BindListView(int category_id)
    {
        ...
        if (!ViewAll)
        {
            ...
            {
                string filename = Server.MapPath(string.Format("/part_comment/{0}_comment.html", dt.Rows[i]["product_serial_no"].ToString()));
    ...
    protected void ListView1_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        ...
        li_store_status.Text = PM.FindStockByLuSku(-1, store_status);
        ...
        try
        {
            ...
            switch (e.CommandName)
            {
                case "Modify":
                    ...
                    pm.Update();
                    //BindListView();
                    InsertTraceInfo(string.Format("modify (price,cost,store_sum) {0}", lu_sku));
                ...
                case "Issue":
                    ...
                    InsertTraceInfo("issue and save (" + lu_sku.ToString() + ")");
                    BindListView();
                    break;
            }
        }
