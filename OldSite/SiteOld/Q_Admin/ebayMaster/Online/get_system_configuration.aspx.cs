...
inner join tb_product p on p.product_serial_no=pd.product_serial_no where p.tag=1 and p.for_sys=1 and p.split_line=0 and pd.showit=1 order by p.short_name_for_sys asc, p.product_ebay_name asc  ", system_sku));
        ...
        EbaySystemModel ebaySys = new EbaySystemModel();
        decimal adjustment = GetEbayPrice.GetEbaySysAdjustment(system_sku, ref ebaySys);
        ...
        decimal sys_price = GetEbayPrice.GetSysPrice(system_sku
            , ref belong_sell
            , ref no_belong_sell
            , ref all_sell);
        ...
        // type
        int flash_type = 1;
        if (ebaySys.is_barebone)
        {
            flash_type = 3;
        }
        else if (ebaySys.is_shrink)
            flash_type = 2;
        else
        {
            flash_type = 1;
