...
<body>
    ...
    <%
        ...
        if not rs.eof then 
            do while not rs.eof 
                response.Write("<input type='radio' name='ship' value='"& rs("categoryId") &"' onclick='cateChange();' ")
                if eBayShippingCategory = rs("categoryId") then response.Write (" checked='checked' ")
                response.Write(" >"& rs("categoryId") & "-- "& rs("ShortCategoryName") &"($"& rs("shippingFee") & ")<br/>")
    ...
    <table>
        <tr>
            <td>
                <br />
                <h3>
                    <input type="checkbox" name="issueToEbay" />Issue to eBay
                    <table>
                        <tr>
                            <td>eBay Category
                            </td>
                        ...
                        <tr>
                            ...
                            <td>
                                <% 
                                    ...
                                    if not rs.eof then
                                        do while not rs.eof 
                                            response.Write "<input type='radio' name='ec' onclick='setEBayCategory("""& rs("eBayCategoryID") &""", """& rs("eBayCategoryName") &""");'>" & rs("eBayCategoryName") &"<br>"
                                        rs.movenext
                                        loop
                                    end if
                                    rs.close : set rs = nothing
            ...
            <td>
                <img src="/pro_img/components/<%=ImgNameSku %>_g_1.jpg" width="300" />
            </td>
        </tr>
    </table>
    <hr size="1" />
    <% 
    set rs = conn.execute("select ebay_code,regdate from tb_ebay_code_and_luc_sku where sku='"& sku &"' order by id desc")
    if not rs.eof then
        Response.write "<p>"
        n =1
        do while not rs.eof 
            ...
            if n = 1 then response.Write "&nbsp;&nbsp;&nbsp;<a href='online/modifyOnlineShippingFee.aspx?sku="+ sku +"&itemid=" + rs("ebay_code") + "' target='_blank' >modify shipping</a>"
            response.Write "<br>"
            n = n + 1
        rs.movenext
        loop
