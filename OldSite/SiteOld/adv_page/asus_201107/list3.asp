	...
	<tr>
    	<td style="background:#ffffff;">
        	<%
				...
				set rs = conn.execute("Select *, case when other_product_sku>0 then other_product_sku else product_serial_no end as img_sku from tb_product where menu_child_serial_no=350 and tag=1  and product_name like 'asus%' and manufacturer_part_number like 'UX%'")
				if not rs.eof then
					do while not rs.eof 
						part_name = rs("product_name")
						long_name = rs("product_name_long_en")
						'if instr(lcase(part_name), "eee")>0 or instr(lcase(part_name), "epc")>0 then 
