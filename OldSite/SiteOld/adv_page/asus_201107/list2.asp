	...
	<tr>
    	<td style="background:#ffffff;">
        	<%
				...
				set rs = conn.execute("Select *, case when other_product_sku>0 then other_product_sku else product_serial_no end as img_sku from tb_product where product_serial_no='20083' or (menu_child_serial_no=350 and tag=1  and product_name like 'asus%') order by product_serial_no desc")
				if not rs.eof then
					do while not rs.eof 
						part_name = rs("product_name")
						long_name = rs("product_name_long_en")
						if rs("product_serial_no") = 20083 or ( instr(lcase(part_name), "eee")>0 or instr(lcase(part_name), "epc")>0 ) then 
