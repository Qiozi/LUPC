...
{
    public static tb_order_helper GetOrderHelperModel(nicklu2Entities context, int orderId)
    {
        var query = context.tb_order_helper.Single(me => me.order_helper_serial_no.Equals(orderId));
        return query;
    }

    public static int GetNewOrderCode(nicklu2Entities context)
    {
        ...
        //return code;
        int code = Code.NewOrderCode(context);
        Config.ExecuteNonQuery("insert into tb_order_code (order_code, regdate, is_order)	values	( '" + code.ToString() + "', now(), 1)");
        return code;
    }


    public static tb_order_helper[] GetModelsByOrderCode(nicklu2Entities context, int order_code)
    {
    ...
    public static tb_order_helper GetModelByOrderCode(nicklu2Entities context, int order_code)
    {
        //OrderHelperModel[] ms = OrderHelperModel.FindAllByProperty("order_code", order_code);
        //if (ms != null)
        //    return ms[0];
        //return null;
        return context.tb_order_helper.FirstOrDefault(me => me.order_code.Value.Equals(order_code));
    }
...
where oh.tag=1 and oh.is_ok=1 order by oh.order_helper_serial_no desc"
    ...
    public void UpdateOutStatus(nicklu2Entities context, int _out_status, int _pre_status, string note, int _id, int pay_status_id)
    {
        ...
        if (note.Trim().Length > 0)
        {
            ...
            //ohm.Update();
            var ohm = context.tb_order_helper.Single(me => me.order_helper_serial_no.Equals(_id));
            ohm.out_note += DateTime.Now.ToString("(MM/dd/yyyy hh:mm:ss)") + note;
            context.SaveChanges();
        }
    }
    public void UpdateOutStatus(nicklu2Entities context, int _out_status, int _pre_status, int _id, int pay_status_id)
    {
        UpdateOutStatus(context, _out_status, _pre_status, "", _id, pay_status_id);
    }
