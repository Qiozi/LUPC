...
public class eBaySystemWorker
{
    ...
    /// <returns></returns>
    public static string ValidateSystem(nicklu2Entities context, string[] partGroupID_sku_commentIDS, decimal new_price)
    {
        try
        {
        ...
        {
            var elmas = new tb_error_log();// ErrorLogModel();
            ...
            context.tb_error_log.Add(elmas);
            context.SaveChanges();
            //elmas.Create();
        }
        return null;
    ...
    /// <param name="new_price"></param>
    /// <returns></returns>
    public static int CreateSys(nicklu2Entities context,
string old_system_sku
        ...
        , ref string itemid)
    {
        string oldItemID = ValidateSystem(context, partGroupID_sku_commentIDS, new_price);
        if (oldItemID != null)
        {
            itemid = oldItemID;
            return 0;
...
,sp.part_group_id,sp.comment_name, '{1}'
 from tb_ebay_system_parts sp inner join tb_product p on p.product_serial_no=sp.luc_sku where sp.system_sku='{0}'"
    ...
    public static void UpdateFlashPrice(nicklu2Entities context, tb_ebay_system ESM
        , decimal sell
        , bool IsChildSystem
        , decimal cost
        , decimal ebayFee
        ...
        , decimal profit)
    {
        //var esm = EbaySystemModel.GetEbaySystemModel(sysSKU);
        if (IsChildSystem)
            ESM.selected_ebay_sell = sell;
        else
            ESM.ebay_system_price = sell;
        if (ESM.is_barebone.Value)
            ESM.selected_ebay_sell = sell;
        ESM.cost = cost;
...
left join tb_part_group pg on pg.part_group_id=pd.part_group_id
    ...
    /// <returns></returns>
    public static FlashType GetFlashType(tb_ebay_system esm)
    {
        if (esm.is_barebone.Value)
            return FlashType.barebone;
        else if (esm.is_shrink.Value)
