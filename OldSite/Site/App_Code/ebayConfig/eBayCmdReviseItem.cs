...
public class eBayCmdReviseItem
{
    ...
    /// <returns></returns>
    public static string Revise(nicklu2Entities context, string itemid, bool isSys
         , string Description
...
", userToken
 ...
 , autoPay == null ? "" : ("<AutoPay>" + (autoPay == true ? "True" : "False") + "</AutoPay>"));
        ...
        // save to db.
        var esxhm = new LU.Data.tb_ebay_send_xml_history();// EbaySendXmlHistoryModel();
    ...
    static void SaveSystemSetting(nicklu2Entities context, int sysSku)
    {

        var querySelling = context
                            .tb_ebay_selling
                            .SingleOrDefault(me =>
                                me.sys_sku.HasValue &&
                                me.sys_sku.Value.Equals(sysSku));
        if (querySelling != null)
        {
            var batch = DateTime.Now.ToString("yyMMddHHmmssfff");
            var query = context.tb_ebay_system_parts
                          .Where(me =>
                              me.system_sku.HasValue &&
                              me.system_sku.Value.Equals(sysSku))
                          .ToList();
            foreach (var item in query)
            {
                var m = new LU.Data.tb_ebay_system_modify_backup
                {
                    Batch = batch,
                    Cost = item.cost.HasValue ? item.cost.Value : 0M,
                    PartSku = item.luc_sku.HasValue ? item.luc_sku.Value : 0,
                    SystemPrice = querySelling.BuyItNowPrice.HasValue ? querySelling.BuyItNowPrice.Value : 0M,
                    Regdate = DateTime.Now,
                    SystemSku = sysSku,
                };
                context.tb_ebay_system_modify_backup.Add(m);
            }
            context.SaveChanges();
        }
    }
    ...
    /// <returns></returns>
    public static string ReviseItem(nicklu2Entities context, string itemid, bool isSys
        , string Description
        , decimal buyItNowPrice
        , int sku
        , string shippingString
...
", userToken
 ...
 , ebayType == eBayModifyType.logo ? (string.IsNullOrEmpty(logoPrictureUrl) ? "" : (string.Format("<PictureDetails><PictureURL>{0}</PictureURL></PictureDetails>{1}", logoPrictureUrl.Replace("http://", "https://"), string.IsNullOrEmpty(eBayTitle) ? "" : "<Title><![CDATA[" + eBayTitle + "]]></Title>"))) : ""
 , !string.IsNullOrEmpty(itemSpecifics) ? itemSpecifics : ""
 , (ebayType == eBayModifyType.storeCategory ?
        (string.IsNullOrEmpty(eBayTitle) ? "" : "<Storefront><StoreCategoryID>" + eBayTitle + "</StoreCategoryID></Storefront>") : (""))
        +
    (ebayType == eBayModifyType.storeCategory ?
        (string.IsNullOrEmpty(cutom_label) ? "" : "<Storefront><StoreCategory2ID>" + cutom_label + "</StoreCategory2ID></Storefront>") : ("")
    )
 ...
 , isSys ? "" : ""
 , GetBaseOfferEnable(isSys)
 , string.IsNullOrEmpty(cutom_label) ? "" : "<SKU><![CDATA[" + cutom_label + "]]></SKU>"
 );

        if (isSys)
        {
            SaveSystemSetting(context, sku);
        }
    ...
    /// <summary>
    /// get ebay add/revise BestOfferDetails
    /// </summary>
    /// <param name="isSys"></param>
    /// <returns></returns>
    public static string GetBaseOfferEnable(bool isSys)
    {
        if (isSys)
        {
            if (System.Configuration.ConfigurationManager.AppSettings["eBayBaseOfferEnableSystem"].ToString() == "1")
            {
                return @"
        ...
        {
            if (System.Configuration.ConfigurationManager.AppSettings["eBayBaseOfferEnablePart"].ToString() == "1")
            {
                return @"
                    <AutoPay>true</AutoPay>
                    <BestOfferDetails>
