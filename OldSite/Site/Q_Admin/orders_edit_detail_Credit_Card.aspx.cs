...
public partial class Q_Admin_orders_edit_detail_Credit_Card : PageBase
{
    ...
    protected void bt_save_credit_card_Click(object sender, EventArgs e)
    {
        ...
        if (ddl_card_country.SelectedValue.ToLower() == "other")
        {
            ...
            new StateShippingModel().SaveNewState(DBContext, cm.customer_card_country_code, cm.customer_card_state_code);
        }
        else
        {
        ...
        var csm = CustomerStoreModel.GetCustomerStoreModel(DBContext, StoreCustomerSerialNo);
        ...
        InsertTraceInfo(DBContext, string.Format("Save Order Price({0}) credit card.", ReqOrderCode));
