...
public partial class Q_Admin_advertisement_page_manage : PageBase
{
   ...
   const int PageSize = 30;
    ...
    protected void btn_create_file_Click(object sender, EventArgs e)
    {
        ...
        if (summary.Length > 0)
        {
            var apm = new tb_advertise_page();// AdvertisePageModel();
            apm.last_regdate = DateTime.Now;
            apm.regdate = DateTime.Now;
            apm.summary = summary;
            DBContext.tb_advertise_page.Add(apm);
    ...
    protected void btn_save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CurrentEditID > 0)
            {
                var apm = DBContext.tb_advertise_page.Single(me => me.id.Equals(CurrentEditID));// AdvertisePageModel.GetAdvertisePageModel(DBContext, CurrentEditID);
                apm.last_regdate = DateTime.Now;
                apm.regdate = DateTime.Now;
                apm.summary = summary;
