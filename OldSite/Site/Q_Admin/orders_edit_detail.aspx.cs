...
public partial class Q_Admin_orders_edit_detail : PageBase
{
    ...
    private void BindBaseInfo()
    {
        ...
        var customers = CustomerStoreModel.FindModelsByOrderCode(DBContext, OrderCode);
        if (customers.Length > 0)
        {
            var customer = customers[0];
            CurrentCustomerID = customer.customer_serial_no.Value;
            ...
            this.lbl_pay_method_text.Text = PayMethodNewModel.GetPayMethodNewModel(DBContext, customer.pay_method.Value).pay_method_name;
...
from tb_order_helper where order_code='{0}'", order_code));
    ...
    protected void btn_submit_to_customer_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                var cmm = new LU.Data.tb_chat_msg();// ChatMsgModel();
    ...
    {
        this.dl_msg_list.DataSource = ChatMsgModel.FindModelsByOrderCode(DBContext, OrderCode);
        ...
        try
        {
            ...
            //if (!SetSystemProduct(_product_id))
            {
                ...
                else
                {
                    ...
                    var order = new LU.Data.tb_order_product();// OrderProductModel();

                    order.menu_child_serial_no = product.menu_child_serial_no;
                    order.order_code = OrderCode;
                    order.order_product_cost = ConvertPrice.Price(CC, product.product_current_cost.Value);
                    order.order_product_price = ConvertPrice.Price(CC, product.product_current_price.Value);
                    ...
                    order.order_product_sold = ConvertPrice.Price(CC, product.product_current_price.Value - product.product_current_discount.Value);// ProductModel.FindOnSaleDiscountByPID(_product_id);
                    ...
                    order.product_current_price_rate = ConvertPrice.Price(CC, product.product_current_price.Value);
                    //  order.Create();
                    DBContext.tb_order_product.Add(order);
                    DBContext.SaveChanges();

                    CH.Alert(KeyFields.SaveIsOK, this.lv_part_list);
                    InsertTraceInfo(DBContext, string.Format("insert part({0}) in order({1})", _product_id, OrderCode));
                    //
    ...
    protected void btn_duplicate_sys_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            AccountOrder(OrderCode);
            InsertTraceInfo(DBContext, string.Format("duplicate system{1} to order({0})", OrderCode, system_tmp_sku));
            CH.CloseParentWatting(this.lv_sys_list);
    ...
    protected void btn_add_sys_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            InsertTraceInfo(DBContext, string.Format("add system{1} to order({0})", OrderCode, system_tmp_sku));
            CH.CloseParentWatting(this.lv_sys_list);
            // CH.Alert(KeyFields.SaveIsOK, this.lv_sys_list);
        }
        catch (Exception ex)
    ...
    private void SetProductToSystemDetail(int system_code, int product_id, int part_quantity)
    {
        ...
        var m = new LU.Data.tb_sp_tmp_detail();// SpTmpDetailModel();
        var sdms = SpTmpDetailModel.GetModelsBySysTmpCode(DBContext, system_code.ToString());
        var singlem = new LU.Data.tb_sp_tmp_detail();// SpTmpDetailModel();
        if (sdms.Length > 0)
            singlem = sdms[0];

        var p = ProductModel.GetProductModel(context, product_id);
      ...
      //  var pgdm = new tb_part_group_detail();
        ...
        context.tb_sp_tmp_detail.Add(m);
        context.SaveChanges();

        var opsdm = new LU.Data.tb_order_product_sys_detail();// OrderProductSysDetailModel();
        ...
        //opsdm.Create();
        context.tb_order_product_sys_detail.Add(opsdm);
        context.SaveChanges();
        InsertTraceInfo(context, string.Format("add part({2}) to system({1}) in order({0})", OrderCode, system_code, product_id));
...
select order_product_price, order_product_cost, order_product_sold, save_price, product_current_price_rate from tb_order_product where  product_serial_no='{0}';", system_code));
        ...
        try
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "DeleteSys":
                    ...
                    InsertTraceInfo(DBContext, string.Format("delete to system({1}) in order({0})", OrderCode, _serial_no));
                    break;
            }
            // CH.Alert("OK", this.lv_sys_list);
            CH.CloseParentWatting(this.lv_sys_list);
    ...
    protected void btn_save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            InsertTraceInfo(DBContext, string.Format("Save Order({0}).", OrderCode));
            AccountOrder(OrderCode);
    ...
    private void AccountOrder(string order_code, bool is_save)
    {
        ...
        int stateid = StateShippingModel.FindStatIDByCode(DBContext, StateID);
...
select 	prod_shipping_fee_id, prod_Sku, is_system, shipping_fee_us, shipping_fee_ca, regdate
	...
	where prod_sku='{0}' and is_system='{1}'", dr["product_serial_no"].ToString(), 0));
            ...
            _cost += p.product_current_cost.Value * product_sum;
...
select 	prod_shipping_fee_id, prod_Sku, is_system, shipping_fee_us, shipping_fee_ca, regdate
	...
	where prod_sku='{0}' and is_system='{1}'", product_id, 0));
        ...
        try
        {
            if (ap_real_count > 0)
            {
                if (ShippingCompanyID != -1)
                {
                    ...
                    var ac = new Account(DBContext, aps, stateid);
        ...
        // tax
        var state = StateShippingModel.GetStateShippingModel(DBContext, stateid);
        ...
        , decimal weee_charge)
    {
        try
        {
            var state = StateShippingModel.GetStateShippingModel(DBContext, state_shipping);
            var model = OrderHelperModel.GetModelsByOrderCode(DBContext, order_code);

            for (int i = 0; i < model.Length; i++)
            {
                var css = CustomerStoreModel.FindModelsByOrderCode(DBContext, order_code.ToString());
                ...
                decimal _gst = (_sub_charge) * state.gst.Value / 100;
                decimal _pst = (_sub_charge) * state.pst.Value / 100;
                model[i].gst = 0M;
                model[i].pst = 0M;
                model[i].hst = 0M;
                ...
                _sale_tax = model[i].gst.Value + model[i].pst.Value + model[i].hst.Value;
                ...
                BindPayRecordLV(model[i].order_code.Value);
                InsertTraceInfo(DBContext, string.Format("Save Order Price({0}).", OrderCode));
            }
        }
        catch (Exception ex)
        {
    ...
    protected void lb_remove_order_ship_charge_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            InsertTraceInfo(DBContext, string.Format("remove lock_shipping_charge in Order({0}).", OrderCode));
            AccountOrder(OrderCode);
        }
        catch (Exception ex)
        {
    ...
    protected void lb_remove_order_discount_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            InsertTraceInfo(DBContext, string.Format("remove lock_input_order_discount in Order ({0}).", OrderCode));
            AccountOrder(OrderCode);
        }
        catch (Exception ex)
        {
...
from tb_order_helper where order_code='{0}'", OrderCode));
    ...
    private void BindOrderStatus()
    {
        ...
        this.ddl_back_status.DataSource = FactureStateModel.FindModelsByShowit(DBContext);
    ...
    {
        var ohm = OrderHelperModel.GetModelsByOrderCode(DBContext,
            int.Parse(OrderCode));
        if (ohm.Length > 0)
        {
    ...
    protected void btn_add_part_by_name_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            var pm = new LU.Data.tb_product();// ProductModel();
            ...
            InsertTraceInfo(DBContext, "Insert new part to IssueStore (in order[" + OrderCode + "]): " + pm.product_serial_no.ToString());
    ...
    protected void btn_save_weee_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (weee_charge >= 0M)
            {
                ...
                AccountOrder(OrderCode);
                InsertTraceInfo(DBContext, string.Format("Update WEEE ({0}) in order({1})", weee_charge, OrderCode));
            }
            CH.CloseParentWatting(this.lv_part_list);
        }
        catch (Exception ex)
