...
<head>
    ...
    <script src="/js_css/jquery_lab/jquery-3.2.1.js" type="text/javascript"></script>
    ...
    <script type="text/javascript">
        ...
        function uploadLogoToEbay(index) {
            //title = 'diff'       
            // var length = $('.modifyLogo').length;
            $('.modifyLogo').each(function (i) {
                var the = $(this);
                the.css({ "color": "white", "background": "green" });
        ...
        $(function () {
            $('.uploadLogoToEbay').on('click', function () {
                uploadLogoToEbay(0);
            });
        });
         ...
         * 获取警告信息
         */
        function generateWarnInfoByDesc(index) {
            $('td').each(function (index, item) {
                var _this = $(this);
                var sku = parseInt(_this.attr('data-sku'));
                if (sku > 0) {
                    _this.css({ 'background': '#f2f2f2' });
                    $.ajax({
                        type: "Get",
                        url: "/q_admin/ebaymaster/ebay_system_cmd.aspx",
                        data: "cmd=generateSysWarnInfoByDesc&luc_sku=" + sku + "&" + new Date().valueOf(),
                        success: function (msg) {
                            if (msg.indexOf("OK") > -1)
...
<body style="padding: 20px; font-size: 10pt; line-height: 20px;">
    ...
    <input type="button" class="uploadLogoToEbay" value="upload logo to eBay" />
    ...
    <input type="button" onclick="generateWarnInfoByDesc();" value="Generate Part Warn" />
				...
				do while not prs.eof
					...
					if not rs.eof then
						...
						do while not rs.eof 
                            ...
                            SpecificsCount = ""
                            if rs("SpecificsCount")>0 then
                                SpecificsCount = rs("SpecificsCount")
                            end if
							...
							response.write "<tr "& bgColorString &" >"&_
                                            ...
                                            "       <td width='15'><a href='/q_admin/ebayMaster/Online/ModifyOnlinePrice.aspx?issystem=0&IsDesc=0&onlyprice=0&LogoPrictureUrl=.jpg&sku="& rs("part_sku") &"&itemid="& rs("itemid") &"' tag='"&rs("itemid")&"' class='modifyLogo' title='ModifyLogo' onclick=""js_callpage_cus(this.href, 'ebay_logo_"& rs("part_sku")& "', 400, 400); $(this).css({'color':'white', 'background':'black'});return false;"">L</a></td>"&_
											...
											"		<td><a href=""/q_admin/ebayMaster/ebay_part_temp_page_view_noShipping.aspx?sku="& rs("part_sku") &""" target='_blank' style='margin-left: 3em;' onclick=""js_callpage_cus(this.href, 'ebay_part_comment_edit', 1050, 700);return false;""> Page </a></td>"&_
