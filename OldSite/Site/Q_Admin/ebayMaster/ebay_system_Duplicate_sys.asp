...
<body>
    ...
    dim exec_sql
    dim new_id
    Dim category_id, caseSku, vcSku, ssdSku, ssdM2Sku, hddSku, memorySku, isCurrPage
...
'	do while not prs.eof
    ...
    caseSku     = SQLescape(request("caseSku"))
    vcSku       = SQLescape(request("vcSku"))
    ssdSku      = SQLescape(request("ssdSku"))
    ssdM2Sku    = SQLescape(request("ssdM2Sku")) 
    hddSku      = SQLescape(request("hddSku"))
    memorySku   = SQLescape(request("memorySku"))
    isCurrPage  = SQLescape(request("isCurrPage"))
    ...
    exec_sql = "insert into tb_ebay_system "&_
	            ...
	            " large_pic_name, "&_
	            " adjustment, "&_
	            " is_issue, now(), batch, caseType, is_barebone,"&_
                " is_disable_flash_customize, id"&_
                " from tb_ebay_system where id="& system_sku 
    ...
    if(caseSku <>"") then     
        conn.execute("update tb_ebay_system_parts set luc_sku='"& caseSku &"' where system_sku='"& new_id &"' and comment_id='9'")
    end if

    if(vcSku<>"") then
        conn.execute("update tb_ebay_system_parts set luc_sku='"& vcSku &"' where system_sku='"& new_id &"' and comment_id='4'")
    end if

    if(ssdSku<>"") then
        conn.execute("update tb_ebay_system_parts set luc_sku='"& ssdSku &"' where system_sku='"& new_id &"' and comment_id='45'")
    end if
    
    if(ssdM2Sku<>"") then
        conn.execute("update tb_ebay_system_parts set luc_sku='"& ssdM2Sku &"' where system_sku='"& new_id &"' and comment_id='46'")
    end if

    if(hddSku<>"") then
        conn.execute("update tb_ebay_system_parts set luc_sku='"& hddSku &"' where system_sku='"& new_id &"' and comment_id='32'")
    end if
    
    if(memorySku<>"") then
        conn.execute("update tb_ebay_system_parts set luc_sku='"& memorySku &"' where system_sku='"& new_id &"' and comment_id='2'")
    end if
'	prs.movenext
'	loop
'	end if
...
Closeconn() %>
    ...
    <script type="text/javascript">
        if ('1' === '<%= isCurrPage%>'){
            location.href = "/q_admin/ebayMaster/lu/ebay_system_edit_2.asp?ebay_system_sku=<%=  new_id %>&category_id=<%= category_id %>&cmd=modify&IsParent=false";
        }
        else if ('' !== '<%= caseSku%>') {
            parent.location.href = "/q_admin/ebayMaster/lu/ebay_system_edit_2.asp?ebay_system_sku=<%=  new_id %>&category_id=<%= category_id %>&cmd=modify&IsParent=false";
        } else {
