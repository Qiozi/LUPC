...
system_sku = SQLescape(request("system_sku"))
 ...
 <input type="submit" value="save" />
            ...
            <td>
                <table>
                    <tr>
                        ...
                        <td>
                            <select name="storeCategory" id="storeCategory"></select></td>
                    </tr>
                    <tr>
                        <td>Store Category2</td>
                        <td>
                            <select name="storeCategory2" id="storeCategory2"></select></td>
                    </tr>

                </table>
                <div id="resultComment"></div>
                <input type="button" id="issueSys" value="issue system onto eBay" />
                <iframe name='iframe100' id='iframe100' style='width: 110px; height: 110px; display: none;' frameborder="0"></iframe>
            </td>
        </tr>
    </table>
    ...
    <!-- <iframe id="iframeSpecific" name="iframeSpecific" src="/q_admin/manager/product/editSpecifics.aspx?sku=<%= system_sku %>&cid=179" style="width: 98%; height: 400px; border: 1px solid #ccc; margin: 1rem;"></iframe>-->
    ...
    <script type="text/javascript">
        ...
        $().ready(function () {
            ...
            $('#issueSys').bind('click', function () {
                ...
                localStorage.setItem("storeyCategory1", storeCategory1)
                localStorage.setItem("storeCategory2", storeCategory2);

                $.ajax({
                    type: "get",
                    url: "/q_admin/ebaymaster/online/AddItem2.aspx?cmd=addsys&system_sku=<%=system_sku %>&storeCategory1=" + storeCategory1 + "&storeCategory2=" + storeCategory2 + "&eBayStoreCateId=" + $('#eBayStoreCateId').val(),
                    data: "",
            ...
            $.ajax({
                ...
                success: function (msg) {
                    ...
                    $('select[name=storeCategory]').each(function () {
                        var id = localStorage.getItem("storeyCategory1");
                        $(this).val(id);
                    });

                    $(msg).appendTo($('select[name=storeCategory2]'));
                    $('select[name=storeCategory2]').each(function () {
                        var id = localStorage.getItem("storeyCategory2");
                        $(this).val(id);
                    });
                },
