...
public partial class Q_Admin_ebayMaster_LU_eBay_Sys_Category_Edit : PageBase
{
    ...
    protected void btn_submit_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (ReqID > 0)
            {
                var pc = ProductCategoryNewModel.GetProductCategoryNewModel(DBContext, ReqID);
            ...
            {
                var pc = new tb_product_category_new();// ProductCategoryNewModel();
                pc.category_name = cate_name;
                pc.priority = priority;
                pc.showit = showit;
                DBContext.tb_product_category_new.Add(pc);
                DBContext.SaveChanges();
            }

            Config.ExecuteNonQuery(@"delete from tb_ebay_templete_and_category where sys_category_id='"+ ReqID.ToString()+@"';");

            var etc = new tb_ebay_templete_and_category();// EbayTempleteAndCategoryModel();
            etc.sys_category_id = ReqID;
            etc.templete_id = int.Parse(this.ddl_templete.SelectedValue.ToString());
            DBContext.tb_ebay_templete_and_category.Add(etc);
            DBContext.SaveChanges();
