...
<!--#include virtual="/q_admin/funs.asp"-->
     ...
     <%
        ...
        if(cmd = "PartGroupDetail") then
		    ...
		    set rs = conn.execute("select * from tb_ebay_system_group_parts where part_group_id = '"& part_group_id &"'")
			if not rs.eof then
			    dim isWarn 
                isWarn = null
				do while not rs.eof
		...
		if(cmd = "CreatePartGroupDetail") then			
            set rs = conn.execute("Select p.product_current_cost, p.product_serial_no, "&_
                            " p.ltd_stock, "&_
							" case when length(p.product_ebay_name)>4 then p.product_ebay_name "&_
							"      when length(p.product_name_long_en )>4 then p.product_name_long_en "&_
							...
							"	   else p.product_short_name end as product_name, p.product_current_price price, pd.part_group_id "&_
						" from tb_part_group_detail pd inner join tb_product p on p.product_serial_no=pd.product_serial_no "&_
						" where p.split_line=0 and p.tag=1 and pd.showit=1 and p.for_sys=1 order by p.product_ebay_name asc, p.product_current_cost asc ")
		
			if not rs.eof then
                conn.execute("delete from tb_ebay_system_group_parts")
				...
				do while not rs.eof
                    ...
                    if not srs.eof then
                        if srs(0) >0 then 
                            product_name = IsWarnPart(rs("product_serial_no"))
                        end if
                    end if
                    srs.close: set srs = nothing

                    if isnull(product_name) or product_name="" then product_name = rs("product_name")	
                    'response.Write(product_name & "<br>")
                    conn.execute("insert into tb_ebay_system_group_parts(product_serial_no,price,product_current_cost,ltd_stock,product_name,part_group_id) VALUES ('"& rs("product_serial_no") &"','"&rs("price")&"','"&rs("product_current_cost")&"','"&rs("ltd_stock")&"','"& SQLescape(product_name)&"','"& part_group_id &"')")
