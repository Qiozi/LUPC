...
public partial class Q_Admin_ebayMaster_Online_ModifyOnlineShippingFee : PageBase
{
    ...
    bool GenerateFee()
    {
        ...
        var ESM = EbaySystemModel.GetEbaySystemModel(DBContext, ReqSKU);

        if (ESM != null && ESM.id > 0)    // is system
        {
            IsSystem = true;
            if (ESM.is_barebone.Value || ESM.is_shrink.Value)
                buyItNowPrice = ESM.selected_ebay_sell.Value;
            else
                buyItNowPrice = ESM.ebay_system_price.Value;
        }
        ...
        else
        {
            ...
            IsNotebook = ProductCategoryModel.IsNotebook(DBContext, PM.menu_child_serial_no.Value);
            if (IsNotebook)
            {
                if (this.ddl_domestic_services_2.SelectedValue == "CA_UPSStandardCanada")
                {
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        //return;
        CH.Alert(eBayCmdReviseItem.Revise(DBContext, ReqItemID, IsSystem, null, 0M, ReqSKU, shippingString, null), this.Literal1);
    }
    ...
    private string GetShippingDetail()
    {
        ...
        return eBayShipping.GetShippingDetail2(DBContext,
            IsNotebook
            , ReqSKU
            , IsSystem
            , ESM == null ? false : (ESM.is_shrink.Value || ESM.is_barebone.Value)
