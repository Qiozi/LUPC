...
public partial class Q_Admin_EditPartDetail : PageBase
{
    ...
    {
        FileHelper.GenerateFile(string.Format("{0}{1}_comment.html", Config.Part_Comment_Path, sku), desc);
        FileHelper.GenerateFile(string.Format("{0}{1}_comment_short.html", Config.Part_Comment_Path, sku), desc_short);
        InsertTraceInfo(DBContext, string.Format("Create file {0}", string.Format("{0}/{1}_comment.html", Config.Part_Comment_Path, sku)));
    }
...
");
    ...
    private void BindPageControl(int lu_sku)
    {
        ...
        if (ReqCmd == EditPartCmd.modifyComment)
        {
            ...
            string commFullFile = string.Format("{0}{1}_comment.html", Config.Part_Comment_Path, lu_sku);
            if (System.IO.File.Exists(commFullFile))
            {
                this.txt_desc.Text = FileHelper.ReadFile(commFullFile); ;
            }
            commFullFile = string.Format("{0}{1}_comment_short.html", Config.Part_Comment_Path, lu_sku);
            if (System.IO.File.Exists(commFullFile))
            {
        ...
        if (Sku > 0)
        {
            ...
            for (int i = 0; i < this.gv_inc_info.Items.Count; i++)
            {
                ...
                if (other_inc_sku.Length > 0)
                {
                    ...
                    DBContext.tb_other_inc_match_lu_sku.Add(oims);
    ...
    protected void btn_save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Sku != -1)
                pm = ProductModel.GetProductModel(DBContext, Sku);

            if (ReqCmd != EditPartCmd.modifyComment)
            {
                pm.product_size_id = int.Parse(this.ddl_size.SelectedValue.ToString());
                pm.export = this.cb_export.Checked;
                pm.hot = sbyte.Parse(this.cb_hot.Checked ? "1" : "0");
                pm.is_non = sbyte.Parse(this.cb_non.Checked ? "1" : "0");
                ...
                pm.@new = sbyte.Parse(this.cb_new.Checked ? "1" : "0");
                ...
                sbyte img_sum;
                sbyte.TryParse(this.txt_product_img_sum.Text, out img_sum);
                ...
                pm.last_regdate = DateTime.Now;
                pm.split_line = sbyte.Parse(this.cb_split_line.Checked ? "1" : "0");
                pm.supplier_sku = this.txt_supplier_sku.Text.Trim();
                pm.tag = sbyte.Parse(this.cb_showit.Checked ? "1" : "0");
                pm.UPC = this.txt_UPC.Text.Trim();
                pm.for_sys = this.cb_for_sys.Checked;
                pm.is_fixed = this.cb_fixed.Checked;
                ...
                else
                {
                    if (CurrentCategoryID > 0)
                    {
                        ...
                        InsertTraceInfo(DBContext, "Create Part Info...:" + pm.product_serial_no.ToString());
                    }
                    else
                    {
            ...
            if (ReqCmd != EditPartCmd.ModifyAndNoComment)
            {
                ...
                {
                    lblWarn.Text = "";
                    //var pdm = new tb_part_comment();// ProductDescModel();
                    ProductDescModel.SavePartComment(DBContext, pm.product_serial_no, comment, comment_short);
                    ...
                    if (ReqCmd == EditPartCmd.modifyComment)
                    {
                        ...
                        //
                        sbyte img_quantity;
                        sbyte.TryParse(this.txt_img_quantity.Text, out img_quantity);
                    ...
                    result += " Part Save Success (" + Sku.ToString() + ").<br>";
                    InsertTraceInfo(DBContext, "Modify Part Info...:" + pm.product_serial_no.ToString());
                }
