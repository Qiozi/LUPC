...
public partial class Q_Admin_orders_edit_detail_customer_msg : PageBase
{
    ...
    string BindSendMailContent()
    {
        ...
        if (cmms != null)
        {
            ...
            sb.Append("</table>");
            sb.Append(new OrderMailContent(DBContext).SendContent(ReqOrderCode.ToString()));
    ...
    {
        string content = "";
        var list = ChatMsgModel.FindModelsByOrderCode(DBContext, ReqOrderCode.ToString());
        if (list != null)
        {
    ...
    void BindMail()
    {
        var CS = CustomerStoreModel.FindByOrderCode(DBContext, ReqOrderCode.ToString());
