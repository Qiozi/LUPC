...
public partial class Q_Admin_inc_get_ebay_price_info : PageBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (ReqSku > 0 && string.IsNullOrEmpty(ReqCmd))
            {
                ...
                var pm = sku.ToString().Length == 6 ? null : ProductModel.GetProductModel(DBContext, (int)sku);
                if (pm != null)
                {
                    eBayPriceHelper eH = new eBayPriceHelper();
                    if (pm.menu_child_serial_no.HasValue && ProductCategoryModel.IsNotebook(DBContext, pm.menu_child_serial_no.Value))
                    {
                        #region notebook
                        ...
                        decimal ebayPrice = eH.eBayNetbookPartPrice(pm.product_current_cost.Value,
                            pm.screen_size.HasValue ? pm.screen_size.Value : 0M,
                            pm.adjustment.Value
                    ...
                    else
                    {
                        ...
                        decimal ebayPrice = GetEbayPrice.GetPartEbayPrice(DBContext, pm, GeteBayOnsalePriceAdjust.GetEbayOnsalePrice(pm.product_serial_no)
                            , ref shipping_fee, ref profit, ref ebay_fee, ref bank_fee);
                        if (ReqShowTitle)
                        {
                            Response.Write("<h2>" + pm.product_ebay_name + "</h2>");
                ...
                else if (sku.ToString().Length == 6)
                {
                    ...
                    var esm = EbaySystemModel.GetEbaySystemModel(DBContext, (int)sku);
    ...
    where p.tag=1 and p.split_line=0 and system_sku='" + sku.ToString() + "'").ToString());
                    ...
                    {
                        ebay_price = GetEbayPrice.GetEbaySysPrice(sys_cost, esm.adjustment.Value
                            , ref profits
                            , ref ebay_fee
                            , ref shipping_fee
                            , true);
    ...
    where p.tag=1 and p.split_line=0 and system_sku='" + sku.ToString() + "' and p.product_serial_no <> 16684 order by ec.priority asc");
            ...
            {
                var pm = ProductModel.GetProductModel(DBContext, (int)ReqSku);
                eBayPriceHelper eH = new eBayPriceHelper();
                ...
                decimal ebayPrice = eH.eBayNetbookPartPrice(pm.product_current_cost.Value, pm.screen_size.Value, pm.adjustment.Value
                    , ref shipping_fee, ref profit, ref ebay_fee, ref bank_fee);
            ...
            ,China_Price_Current:'{14}'}}"
                    ...
                    , pm.product_current_cost.Value.ToString("##.00")
                    , bank_fee.ToString("##.00")
                    , pm.adjustment.Value.ToString("##.00")
                    , shipping_fee.ToString("##.00")
                    , profit.ToString("##.00")
                    , ebay_fee.ToString("##.00")
                    , pm.curr_change_ltd
                    , pm.curr_change_cost.Value.ToString("##.00")
