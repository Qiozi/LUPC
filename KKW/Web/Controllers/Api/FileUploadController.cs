...
namespace Web.Controllers.Api
{
    ...
    public class FileUploadController : ApiController
    {
        ...
        [HttpPost]
        public async Task<string> Post(Guid token, int userId, string userName)
        {
            try
            {
                var context = new DB.qstoreEntities();
                var query = context.tbtokens.SingleOrDefault(me => me.Gid.Equals(token));
                if(query == null)
                {
                    return "上传失败, Gid is falid.";
                }
                else
                {
                    query.CreateTime = DateTime.Now.AddDays(1);                  
                }
                ...
                return await Request.Content
                                ...
                                .ContinueWith(t =>
                                {
                                    ...
                                    {
                                        string oldFileName = Newtonsoft.Json.JsonConvert.DeserializeObject<string>(file.Headers.ContentDisposition.FileName);

                                        var filename = file.LocalFileName;
                                        ReadExcel(context, filename, userId, userName);
                                    }
