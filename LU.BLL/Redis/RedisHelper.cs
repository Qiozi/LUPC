...
namespace LU.BLL.Redis
{
    public class RedisHelper
    {
        ...
        public RedisHelper(int dbNum, string customKey)
            : this(dbNum, null, customKey)
        {
        }

        public RedisHelper(int dbNum, string readWriteHosts, string customKey)
        {
            // custom key 不用， 因为微信请求的域名固定，会与测试的域名有差异，引起读写不一至。
            // CustomKey = customKey;
            CustomKey = "";
            DbNum = dbNum;
            _conn =
        ...
        /// <returns></returns>
        public RedisValue[] StringGet(List<string> listKey, string customKey)
        {
